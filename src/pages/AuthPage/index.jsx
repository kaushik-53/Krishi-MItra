...
export default function AuthPage() {
    ...
    const formVariants = {
        ...
        exit: { opacity: 0, x: isLogin ? 50 : -50, transition: { duration: 0.3, ease: 'easeIn' } }
    };
    ...
    return (<div className="min-h-screen bg-surface-0 flex relative overflow-hidden">
      {/* Animated Background Blobs */}
      <motion.div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-primary-400/20 blur-[100px] pointer-events-none" animate={{ scale: [1, 1.2, 1], x: [0, 50, 0], y: [0, 30, 0] }} transition={{ duration: 15, repeat: Infinity, ease: 'easeInOut' }}/>
      <motion.div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-amber-400/10 blur-[120px] pointer-events-none" animate={{ scale: [1, 1.3, 1], x: [0, -50, 0], y: [0, -40, 0] }} transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut', delay: 2 }}/>
      ...
      <div className="w-full max-w-[1400px] mx-auto flex items-center justify-center lg:justify-between px-4 lg:px-12 py-8 relative z-10">
        ...
        <div className="hidden lg:flex flex-col justify-center w-1/2 pr-12">
          <Link to="/" className="inline-flex items-center gap-3 mb-12">
            <motion.div whileHover={{ rotate: 180 }} transition={{ duration: 0.5 }} className="w-12 h-12 rounded-2xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-2xl shadow-lg shadow-primary-500/30">
              🌾
            </motion.div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={isLogin ? 'login-text' : 'register-text'} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
        ...
        <div className="w-full max-w-md lg:w-1/2 flex justify-center">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="w-full glass-card p-8 sm:p-10 shadow-2xl relative overflow-hidden">
            ...
            <Button variant="secondary" fullWidth onClick={handleGoogle} isLoading={isLoading} className="mb-6 h-12 shadow-sm hover:shadow-md transition-shadow" leftIcon={<svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>}>{t('auth.google')}</Button>
            ...
            <div className="relative overflow-hidden min-h-[200px]">
              <AnimatePresence mode="wait">
                {isLogin ? (<motion.form key="login-form" variants={formVariants} initial="hidden" animate="enter" exit="exit" onSubmit={loginForm.handleSubmit(handleLoginSubmit)} className="space-y-5">
                    ...
                    <div className="flex justify-end -mt-2">
                      <button type="button" onClick={() => setShowForgotModal(true)} className="text-xs text-primary-500 hover:text-primary-400 transition-colors font-medium">
                        {t('auth.forgotPassword')}
                      </button>
                    </div>
                    <Button type="submit" fullWidth isLoading={isLoading} className="mt-2 h-12">{t('auth.login')}</Button>
                  </motion.form>) : (<motion.form key="register-form" variants={formVariants} initial="hidden" animate="enter" exit="exit" onSubmit={registerForm.handleSubmit(handleRegisterSubmit)} className="space-y-5">
      ...
      <AnimatePresence>
        {showForgotModal && (<motion.div key="forgot-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backdropFilter: 'blur(8px)', backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={(e) => { if (e.target === e.currentTarget)
            setShowForgotModal(false); }}>
            <motion.div key="forgot-modal" initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="w-full max-w-sm glass-card p-8 shadow-2xl relative overflow-hidden">
              {/* top gradient bar */}
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary-400 to-amber-400"/>

              {/* Close button */}
              <button type="button" onClick={() => setShowForgotModal(false)} className="absolute top-4 right-4 text-text-muted hover:text-text-primary transition-colors" aria-label="Close">
                <X className="w-5 h-5"/>
              </button>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <Input label={t('auth.email')} type="email" autoComplete="email" leftIcon={<Mail className="w-4 h-4"/>} value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} placeholder="your@email.com"/>
                <Button type="submit" fullWidth isLoading={forgotLoading} className="h-11">
                  Send Reset Link
                </Button>
              </form>
