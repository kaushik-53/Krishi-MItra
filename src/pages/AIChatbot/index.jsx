...
export default function AIChatbot() {
    ...
    const sendMessage = async (text) => {
        if (!text.trim())
            return;
        const userMsg = { id: generateId(), role: 'user', content: text.trim(), timestamp: new Date() };
        ...
        try {
            ...
            const aiMsg = { id: generateId(), role: 'assistant', content: result.response, timestamp: new Date() };
            ...
            const errorMsg = { id: generateId(), role: 'assistant', content: "I'm sorry, I'm having trouble connecting to the network right now. Please try again later.", timestamp: new Date() };
    ...
    return (<PageWrapper showSidebar>
        ...
        <Card padding="none" className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {messages.length === 0 && (<div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg w-full">
                  {suggestedQuestions.map((q, i) => (<button key={i} onClick={() => sendMessage(lang === 'hi' ? q.hi : q.en)} className="p-3 rounded-xl text-left text-sm bg-surface-2/50 border border-glass-border hover:border-primary-400/30 hover:bg-surface-3 transition-all text-text-secondary">
                      {lang === 'hi' ? q.hi : q.en}
                    </button>))}
                </div>
              </div>)}
            ...
            {/* Messages */}
            {messages.map((msg) => (<motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] ${msg.role === 'user' ? 'order-1' : ''}`}>
                  {msg.role === 'assistant' && (<div className="flex items-center gap-2 mb-1">
                ...
                : 'bg-surface-2 border border-glass-border text-text-primary rounded-tl-md'}`}>
                  ...
                  {msg.role === 'assistant' && (<div className="flex items-center gap-2 mt-1.5">
                      <button onClick={() => handleFeedback(msg.id, 'positive')} className={`p-1 rounded ${msg.feedback === 'positive' ? 'text-success' : 'text-text-muted hover:text-success'}`}>
                        <ThumbsUp className="w-3.5 h-3.5"/>
                      </button>
                      <button onClick={() => handleFeedback(msg.id, 'negative')} className={`p-1 rounded ${msg.feedback === 'negative' ? 'text-danger' : 'text-text-muted hover:text-danger'}`}>
            ...
            {isTyping && (<div className="flex items-center gap-2">
                ...
                <div className="flex gap-1.5 p-3 rounded-2xl bg-surface-2 border border-glass-border">
                  {[0, 1, 2].map((i) => (<motion.div key={i} className="w-2 h-2 rounded-full bg-primary-400" animate={{ y: [0, -6, 0] }} transition={{ duration: 1, repeat: Infinity, delay: i * 0.2 }}/>))}
                </div>
              </div>)}
            <div ref={chatEndRef}/>
          </div>
          ...
          <div className="p-4 border-t border-glass-border">
            <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="flex items-center gap-2">
              <input value={input} onChange={(e) => setInput(e.target.value)} placeholder={t('chat.placeholder')} className="glass-input flex-1" disabled={isTyping}/>
      ...
      <Modal isOpen={showHistory} onClose={() => setShowHistory(false)} title={t('chat.history') || 'Chat History'} size="lg">
        {isLoadingHistory ? (<div className="flex items-center justify-center p-8">
            <Spinner size="lg" className="text-primary-400"/>
          </div>) : historyList.length === 0 ? (<div className="text-center p-8 text-text-muted">
            No chat history found. Start a conversation to save your history!
          </div>) : (<div className="max-h-[60vh] overflow-y-auto space-y-4 pr-2">
            {historyList.map((item) => (<div key={item.id} className={`p-4 rounded-xl border flex items-center justify-between hover:border-primary-400/50 transition-colors cursor-pointer ${activeSessionId === item.id ? 'bg-primary-400/10 border-primary-400/30 text-primary-400' : 'bg-surface-2 border-glass-border text-text-primary'}`} onClick={() => handleSelectSession(item)}>
                ...
                <button onClick={(e) => handleDeleteSession(e, item.id)} className="p-2 rounded-lg text-text-muted hover:text-danger hover:bg-danger/10 transition-colors">
