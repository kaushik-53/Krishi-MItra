    ...
    return (<PageWrapper>
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6 w-full">
        ...
        <motion.div variants={fadeInUp}>
          <Card>
            <div {...getRootProps()} className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 ${isDragActive ? 'border-primary-400 bg-primary-400/5' : 'border-glass-border hover:border-primary-400/50 hover:bg-surface-2/50'}`}>
              <input {...getInputProps()}/>
            ...
            {/* Image previews */}
            {images.length > 0 && (<div className="flex gap-3 mt-4 flex-wrap">
                {images.map((img, i) => (<div key={i} className="relative group">
            ...
            <div className="flex gap-3 mt-4">
              <Button onClick={() => { if (images.length === 0) {
        const input = document.querySelector('input[type="file"]');
        input?.click();
    }
    else {
        handleAnalyze();
    } }} isLoading={isAnalyzing} leftIcon={<Image className="w-4 h-4"/>}>
        ...
        {/* AI Thinking */}
        {isAnalyzing && (<motion.div variants={fadeInUp}><Card><AIThinkingLoader /></Card></motion.div>)}

        {/* Results */}
        <AnimatePresence>
          {result && !isAnalyzing && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex-1">
                    ...
                    <div className="h-3 bg-surface-2 rounded-full overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${result.confidence}%` }} transition={{ duration: 1, ease: 'easeOut' }} className="h-full bg-gradient-to-r from-primary-600 to-primary-400 rounded-full"/>
                    </div>
                  </div>
                </div>
              </Card>
              ...
              <Card>
                ...
                <div className="flex gap-2 mb-4">
                  {['organic', 'chemical'].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeTab === tab ? 'bg-primary-400/15 text-primary-400 border border-primary-400/30' : 'text-text-muted hover:text-text-primary bg-surface-2'}`}>
                      {t(`disease.${tab}`)}
                    </button>))}
                </div>
                <div className="space-y-3">
                  {result.treatments.filter((tr) => tr.type === activeTab).map((tr, i) => (<div key={i} className="p-4 rounded-xl bg-surface-2/50 border border-glass-border">
              ...
              <Card>
                ...
                <ul className="space-y-2">
                  {result.prevention.map((tip, i) => (<li key={i} className="flex items-start gap-2 text-sm text-text-secondary">
      ...
      <Modal isOpen={showHistory} onClose={() => setShowHistory(false)} title={t('disease.history') || 'Detection History'} size="lg">
        {isLoadingHistory ? (<div className="flex items-center justify-center p-8">
            ...
            {historyList.map((item) => (<div key={item.id} className="p-4 rounded-xl bg-surface-2 border border-glass-border flex gap-4 hover:border-primary-400/50 transition-colors cursor-pointer" onClick={() => {
                    setResult({
                        diseaseName: item.diseaseName,
                        confidence: item.confidence,
                        severity: item.severity,
                        description: item.description,
                        treatments: item.treatments.map((tr) => ({
                            type: tr.type,
                            name: tr.name,
                            dosage: tr.dosage,
                ...
                {item.imageUrl && (<img src={item.imageUrl} alt={item.diseaseName} className="w-20 h-20 object-cover rounded-xl border border-glass-border flex-shrink-0"/>)}
                <div className="flex-1">
