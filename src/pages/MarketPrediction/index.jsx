...
export default function MarketPrediction() {
    ...
    const handleSetAlert = () => {
        ...
        setTimeout(() => {
            import('firebase/firestore').then(({ Timestamp }) => {
                ...
                toast.custom((t) => (<div className={`${t.visible ? 'animate-enter' : 'animate-leave'} max-w-md w-full bg-surface-1 shadow-lg rounded-xl border border-primary-500/30 pointer-events-auto flex ring-1 ring-black ring-opacity-5`}>
    ...
    return (<PageWrapper>
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6 max-w-5xl mx-auto">
        ...
        {loading ? (<motion.div variants={fadeInUp} className="flex justify-center py-24">
           ...
           </motion.div>) : prices.length === 0 ? (<motion.div variants={fadeInUp} className="grid grid-cols-1 gap-6 mt-12">
            ...
            <motion.div variants={fadeInUp} className="overflow-hidden rounded-2xl bg-surface-1 border border-glass-border p-3">
              <div className="flex gap-6 animate-marquee">
                {[...recentPrices, ...recentPrices].map((item, i) => (<div key={i} className="flex items-center gap-2 whitespace-nowrap">
                ...
                <Card>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div className="flex gap-2">
                      <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} className="glass-input text-sm py-1.5 px-3 w-auto">
                  ...
                  {chartData.length > 0 ? (<ResponsiveContainer width="100%" height={280}>
                    ...
                    </ResponsiveContainer>) : (<div className="flex items-center justify-center h-[280px] text-text-muted">
              ...
              <motion.div variants={fadeInUp}>
                <Card className="h-full">
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    {recentPrices.map((item, i) => (<div key={i} className="p-3 rounded-xl bg-surface-2/50 hover:bg-surface-3 transition-colors cursor-pointer">
      ...
      <Modal isOpen={isAlertModalOpen} onClose={() => setIsAlertModalOpen(false)} title="Set Price Alert">
        <div className="space-y-4">
          ...
          <Input label="Target Price (₹ per Quintal)" type="number" placeholder="e.g. 2500" value={alertPrice} onChange={(e) => setAlertPrice(e.target.value)} leftIcon={<span className="text-text-muted font-bold ml-1">₹</span>}/>
