...
export default function WeatherAdvisory() {
    ...
    useEffect(() => {
        ...
        async function fetchWeather() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition((pos) => fetchWeatherData(pos.coords.latitude, pos.coords.longitude), (err) => {
    ...
    if (forecast && forecast.list) {
        ...
        hourlyData = forecast.list.slice(0, 6).map((item) => {
        ...
        weekForecast = Object.values(dailyData).slice(0, 5).map((d) => ({
            ...d,
    ...
    return (<PageWrapper>
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6">
        ...
        {!weather && !loading ? (<motion.div variants={fadeInUp}>
          ...
          </motion.div>) : loading ? (<motion.div variants={fadeInUp}>
             <Card className="flex flex-col items-center justify-center py-16 text-center h-full">
            ...
            <motion.div variants={fadeInUp}>
              <Card className="relative overflow-hidden">
                ...
                return (<div key={item.label} className="p-3 rounded-xl bg-surface-2/50 text-center">
            ...
            {forecast && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Temperature Chart */}
                <motion.div variants={fadeInUp}>
                  <Card>
                ...
                <motion.div variants={fadeInUp}>
                  <Card>
                    ...
                    <div className="space-y-2">
                      {weekForecast.map((day) => (<div key={day.day} className="flex items-center justify-between py-2 border-b border-glass-border last:border-0">
            ...
            <motion.div variants={fadeInUp}>
              <Card>
                ...
                rainfall: weather.rain ? weather.rain['1h'] || 0 : 0
            }).length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {weatherService.generateAdvisories({
                    temp: weather.main.temp,
                    humidity: weather.main.humidity,
                    windSpeed: weather.wind.speed * 3.6,
                    rainfall: weather.rain ? weather.rain['1h'] || 0 : 0
                }).map((adv, i) => (<div key={i} className={`p-4 rounded-xl border ${adv.priority === 'high' ? 'bg-danger/5 border-danger/20' : 'bg-warning/5 border-warning/20'}`}>
                  ...
                  </div>) : (<div className="p-4 rounded-xl border bg-success/5 border-success/20 text-success">
