...
export default function FarmProfile() {
    ...
    if (!user)
        return null; // Or some loading state if not loaded
    ...
    return (<PageWrapper>
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6 w-full max-w-5xl mx-auto">
        <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {!isEditing && (<Button variant="outline" size="sm" onClick={() => setIsEditing(true)} leftIcon={<Edit3 className="w-4 h-4"/>}>
        ...
        {isEditing ? (<motion.div variants={fadeInUp} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card>
          ...
          </motion.div>) : !hasFarmData ? (<motion.div variants={fadeInUp}>
          ...
          </motion.div>) : (<motion.div variants={fadeInUp} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <div className="flex flex-col gap-3">
                ...
                {user.farm.secondaryCrops && user.farm.secondaryCrops.length > 0 && (<div className="mt-2">
                    <p className="text-xs text-text-muted mb-2">Secondary Crops</p>
                    <div className="flex flex-wrap gap-2">
                      {user.farm.secondaryCrops.map((crop) => (<Badge key={crop} variant="default">{crop}</Badge>))}
                    </div>
                  </div>)}
              </div>
            </Card>
