...
export default function CTASection() {
    ...
    return (<section className="py-24 relative overflow-hidden" id="cta">
      ...
      <Container size="sm" clean className="relative z-10 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <div className="mt-8 flex items-center justify-center gap-6">
            {['ICAR', 'NABARD', 'PM-KISAN', 'eNAM'].map((org) => (<span key={org} className="text-xs text-text-muted font-mono px-3 py-1 rounded-full border border-glass-border">{org}</span>))}
          </div>
        </motion.div>
      </Container>
    </section>);
