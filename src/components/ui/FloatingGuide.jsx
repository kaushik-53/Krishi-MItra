...
export default function FloatingGuide() {
    ...
    const [guideLang, setGuideLang] = useState(language);
    ...
    useEffect(() => {
        setGuideLang(language);
    }, [language]);
    const toggleLanguage = () => {
        setGuideLang((prev) => (prev === 'en' ? 'hi' : 'en'));
    };
    const detailedGuide = {
        en: {
    ...
    return (<div className="fixed bottom-6 right-4 sm:right-6 z-[100] flex flex-col items-end">
      <AnimatePresence>
        {isOpen && (<motion.div ref={guideRef} initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }} transition={{ duration: 0.2 }} className="mb-4 w-[calc(100vw-2rem)] sm:w-[400px] max-w-full origin-bottom-right">
            <Card className="relative overflow-hidden border-primary-500/30 bg-surface-1/95 backdrop-blur-md shadow-2xl flex flex-col max-h-[70vh]">
              ...
              <div className="flex items-center justify-between mb-3 border-b border-glass-border pb-3 shrink-0">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={toggleLanguage} className="flex items-center gap-1.5 text-xs font-semibold text-text-primary hover:bg-surface-2 transition-colors bg-surface-1 border border-glass-border px-2.5 py-1.5 rounded-lg shadow-sm">
                    <Globe className="w-3.5 h-3.5 text-primary-400"/>
                    {guideLang === 'en' ? 'हिंदी' : 'English'}
                  </button>
                  <button onClick={() => setIsOpen(false)} className="p-1.5 text-text-muted hover:text-red-400 rounded-lg hover:bg-surface-2 transition-colors">
                    <X className="w-4 h-4"/>
                  </button>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4 py-2">
                <h4 className="text-base font-display font-bold text-primary-400 mb-4">{detailedGuide[guideLang].title}</h4>
                <div className="space-y-4">
                  {detailedGuide[guideLang].steps.map((step, index) => (<motion.div key={index} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex gap-3">
      ...
      <motion.button ref={triggerRef} onClick={() => setIsOpen(!isOpen)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`w-14 h-14 rounded-full shadow-2xl flex items-center justify-center relative ${isOpen ? 'bg-primary-600' : 'bg-primary-500'}`}>
